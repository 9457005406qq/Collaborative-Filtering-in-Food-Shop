...
def rcf(topk,n_similar_user,n_similar):
    ...
    u, s, vt = svds(train_matrix, k=90)  # k是分解的纬度
    ...
    prest = np.zeros((n_users_train, n_items_train)) # 餐馆
    puser = np.zeros((n_users_train, n_items_train)) #用户
    ...
    for i in testuserset:
        ...
        # print(userid)
        user_ratings =   0.01*puser[userid, :]+ 0.99*predictions_svd[i, :]#取出pred餐馆 ，p1用户
        # print(user_ratings)
